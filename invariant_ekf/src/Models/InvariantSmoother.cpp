...
#include "Models/InvariantSmoother.hpp"
#include <string>
#include <malloc.h>
...
void Inv_Factors::Batch_Update_n_Get_Gradient_Hess_Cost(bool is_for_marginalization, ROBOT_STATES *RS,
  ...
  if (!is_for_marginalization) {
	...
	for (int i = 0; i <= frame_count; i++) {
	  ...
	  if(RS_temp[i].LVM_In)
	  {
		Invariant_LVM_Measurement_Factor(i, Hessian, gradient, cost);
	  }
	}
  } else {
	...
	if(RS_temp[1].LVM_In)
	{
	  Invariant_LVM_Measurement_Factor(1, Hessian, gradient, cost);
	}
...
void Inv_Factors::Invariant_LVM_Measurement_Factor(int j, Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic> &Hessian,
															  Eigen::Matrix<double, Eigen::Dynamic, 1> &gradient, double &cost)
{
  //residual------------------------------------------------------------------------------
  Eigen::Vector3d residual;

  Eigen::Matrix3d sqrt_information_matrix_LVM;

  sqrt_information_matrix_LVM.setIdentity();

  sqrt_information_matrix_LVM(0, 0) = 1.0 / (fac_info[j].sqrt_LVM(0) + 1e-10);
  sqrt_information_matrix_LVM(1, 1) = 1.0 / (fac_info[j].sqrt_LVM(1) + 1e-10);
  sqrt_information_matrix_LVM(2, 2) = 1.0 / (fac_info[j].sqrt_LVM(2) + 1e-10);

  if (j == 0 && marginalization_flag) {
	residual = fac_info[j].X_LVM - X_0_bar.block(0,0,3,3).transpose()*X_0_bar.block(0, 3, 3, 1);
  } else {
	residual = fac_info[j].X_LVM - RS_temp[j].Rotation.transpose()*RS_temp[j].Velocity;
  }
  Eigen::Vector3d residual_t = residual;
  residual = sqrt_information_matrix_LVM * residual;
...
//  std::cout<<"now meas residual is "<<residual.transpose()*residual<<std::endl;
  ...
  partial_xi_i.block(0, 3, 3, 3) = -sqrt_information_matrix_LVM;
...
void InvariantSmoother::Call_File(double _dt, const std::string &file_name) {
  ...
  myfile.open("../src/raisin_hound/houndlib/Invariant_Smoother_for_Legged_Robot.git/polished_data/" + file_name
				  + "_sensordata.txt");
  ...
  //IS_03
  gt_sd = (int)round(0.04 / _dt);//IS_04
  //IS_05
  //IEKF_01
  //IEKF
...
//	std::cout << cov_val_setting << std::endl;
  ...
  convergence_cond = "(" + to_string_n_signficant_figures(Optimization_Epsilon, 1) + "_"
	  + to_string_n_signficant_figures(Max_Iteration, 2) + ")"
	  + "(" + to_string_n_signficant_figures(backppgn_rate, 2) + "_"
	  + to_string_n_signficant_figures(Max_backpropagate_num, 2) + ")";

  std::string mode;
  if (variable_contact_cov_mode) {
	if (Retract_All_flag) {
	  mode = "(CS" + to_string_n_signficant_figures(cov_amplifier, 2) + ")(RetO)";
	} else {
	  mode = "(CS" + to_string_n_signficant_figures(cov_amplifier, 2) + ")(RetX)";
	}
  } else if (slip_rejection_mode) {
	if (Retract_All_flag) {
	  mode = "(SR" + to_string_n_signficant_figures(slip_threshold, 2) + "_"
		  + to_string_n_signficant_figures(long_term_a_threshold, 2) + ")(RetO)";
	  //mode="(SR" + to_string_n_signficant_figures(slip_threshold,2) + ")(RetO)";
	} else {
	  mode = "(SR" + to_string_n_signficant_figures(slip_threshold, 2) + "_"
		  + to_string_n_signficant_figures(long_term_a_threshold, 2) + ")(RetX)";
	  //mode="(SR" + to_string_n_signficant_figures(slip_threshold,2) + ")(RetX)";
	}
  } else {
	if (Retract_All_flag) {
	  mode = "(RetO)";
  ...
  estimator_info = mode + convergence_cond + "IS";
  initial_info = "(" + to_string_n_figures_under_0(initial_condition(0), 2) + "_"
	  + to_string_n_figures_under_0(initial_condition(1), 2) + "_"
	  + to_string_n_figures_under_0(initial_condition(2), 2) + "_"
	  + to_string_n_figures_under_0(initial_condition(3), 2) + "_"
	  + to_string_n_figures_under_0(initial_condition(4), 2) + "_"
	  + to_string_n_figures_under_0(initial_condition(5), 2) + "_"
	  + to_string_n_figures_under_0(initial_condition(6), 2) + "_"
	  + to_string_n_figures_under_0(initial_condition(7), 2) + "_"
	  + to_string_n_figures_under_0(initial_condition(8), 2) + "_"
	  + to_string_n_figures_under_0(initial_condition(9), 2) + "_"
	  + to_string_n_figures_under_0(initial_condition(10), 2) + "_"
	  + to_string_n_figures_under_0(initial_condition(11), 2) + ")";

  if (textfile_flag) {

	RS[0].Position << GroundTruth[1 + gt_sd][12] + initial_condition(0), GroundTruth[1 + gt_sd][13]
		+ initial_condition(1), GroundTruth[1 + gt_sd][14] + initial_condition(2);
	RS[0].Velocity << GroundTruth[1 + gt_sd][9] + initial_condition(7), GroundTruth[1 + gt_sd][10]
		+ initial_condition(8), GroundTruth[1 + gt_sd][11] + initial_condition(9);
...
void InvariantSmoother::new_measurement(Eigen::Matrix<double, num_z, 1> Sensor_i, Eigen::Matrix<bool, 4, 1> Contact_i) {
  ...
  if (frame_count == 0) {//Initializing states which require sensor measurement necessarily
	...
	for (int k = 0; k < estimator_common_struct_.leg_no; k++) {
	  RS[0].d.block(3 * k, 0, 3, 1) = RS[0].Position + RS[0].Rotation * LeggedRobotKinematics::GetImu2FootPosition(k,
																												   Estimation_Z
																													   .block<
																														   3,
																														   1>(6 + 3 * k,
	...
	for (int k = 0; k < estimator_common_struct_.leg_no; k++) {
	  ...
	  RS[frame_count].d_v.block(3 * k, 0, 3, 1) = RS[frame_count].Velocity
		  ...
		  + RS[frame_count].Rotation * Hat_so3(IMU_Gyro - RS[frame_count].Bias_Gyro)
			  * LeggedRobotKinematics::GetImu2FootPosition(k,
														   ENCODER.block<3, 1>(3 * k, 0),
														   Robot::FootRadiusForEstimator,
  ...
  for (int k = 0; k < estimator_common_struct_.leg_no; k++) {
	if (RS[j].Hard_Contact(k)) {
	  ...
	  kin_k.meas_primitive_sqrt_info =
		  estimator_common_struct_.estimator_covariances_.SQRT_INFO_Covariance_Encoder * LeggedRobotKinematics::GetJacobian(k, ENC).inverse();
  ...
  for (int p = 0; p <= frame_count; p++) {
	for (int k = 0; k < estimator_common_struct_.leg_no; k++) {
	  RS[p].d_v.block(3 * k, 0, 3, 1) = RS[p].Velocity
		  + RS[p].Rotation * LeggedRobotKinematics::GetJacobian(k, Estimation_Z.block<3, 1>(num_z * p + 6 + 3 * k, 0))
		  ...
		  + RS[p].Rotation * Hat_so3(Estimation_Z.block<3, 1>(num_z * p, 0) - RS[p].Bias_Gyro)
			  * LeggedRobotKinematics::GetImu2FootPosition(k,
														   Estimation_Z.block<3, 1>(num_z * p + 6 + 3 * k, 0),
														   Robot::FootRadiusForEstimator,
														   estimator_common_struct_.IMU2BD);
	}
  }
...
void InvariantSmoother::new_measurement(Eigen::Matrix<double, num_z, 1> Sensor_i,
  ...
  if (textfile_flag) {
	...
	RS[frame_count].GPS_In = GPS_In_i;
	RS[frame_count].LVM_In = LVM_In_i;
...
//            RS[frame_count].Hard_Contact(k)  = RS[frame_count].Contact(k)-RS[frame_count].Slip(k);
  ...
  fac_info[j].X_LVM = X_LVM_i;
  fac_info[j].sqrt_LVM =
	  sqrt(estimator_common_struct_.cov_amplifier)
		  * sqrt_LVM_i;
	...
	for (int k = 0; k < estimator_common_struct_.leg_no; k++) {
	  ...
	  RS[i].d_v.block(3 * k, 0, 3, 1) = RS[i].Velocity
		  ...
		  + RS[i].Rotation * Hat_so3(Estimation_Z.block<3, 1>(num_z * i, 0) - RS[i].Bias_Gyro)
			  * LeggedRobotKinematics::GetImu2FootPosition(k,
														   Estimation_Z.block<3, 1>(num_z * i + 6 + 3 * k, 0),
														       Robot::FootRadiusForEstimator,
														   estimator_common_struct_.IMU2BD);
...
void InvariantSmoother::SaveOneStep(int cnt) {
  if (cnt < SAVEMAXCNT) {
	if (textfile_flag) {
	  ...
	  Eigen::Vector3d temp_eul = Rotation_to_EulerZYX(temp_R);

	  for (int i = 0; i < 3; i++) {
		for (int k = 0; k < 3; k++) {
		  SAVE_BUFFER[idx_TRUE_Rotation + i * 3 + k][cnt] = GroundTruth[j + gt_sd][3 * i + k];
		}
	  }

	  for (int i = 0; i < 3; i++) {
		SAVE_BUFFER[idx_TRUE_Position + i][cnt] = GroundTruth[j + gt_sd][12 + i];
		SAVE_BUFFER[idx_TRUE_Velocity + i][cnt] = GroundTruth[j + gt_sd][9 + i];
		SAVE_BUFFER[idx_TRUE_Bias_Gyro + i][cnt] = 0;
		SAVE_BUFFER[idx_TRUE_Bias_Acc + i][cnt] = 0;
		SAVE_BUFFER[idx_TRUE_rpy + i][cnt] = temp_eul(i);
	  }
	  ...
	  for (int i = 0; i < estimator_common_struct_.leg_no; i++) {
		...
		v_true << GroundTruth[j + gt_sd][9], GroundTruth[j + gt_sd][10], GroundTruth[j + gt_sd][11];
		R_true << GroundTruth[j + gt_sd][0], GroundTruth[j + gt_sd][1], GroundTruth[j + gt_sd][2],
			GroundTruth[j + gt_sd][3], GroundTruth[j + gt_sd][4], GroundTruth[j + gt_sd][5],
			GroundTruth[j + gt_sd][6], GroundTruth[j + gt_sd][7], GroundTruth[j + gt_sd][8];

		dv_true = v_true
			+ R_true
				* LeggedRobotKinematics::GetJacobian(i, Estimation_Z.block<3, 1>(num_z * frame_count + 6 + 3 * i, 0))
				* Estimation_Z.block<3, 1>(num_z * frame_count + num_z_imu + num_z_encoder + 3 * i, 0)
			+ R_true * Hat_so3(Estimation_Z.block<3, 1>(num_z * frame_count, 0))
				* LeggedRobotKinematics::GetImu2FootPosition(i,
															 Estimation_Z
																 .block<3, 1>(num_z * frame_count + 6 + 3 * i, 0),
																     Robot::FootRadiusForEstimator,
															 estimator_common_struct_.IMU2BD);

		if (dv_true.norm() > estimator_common_struct_.slip_threshold) {
		  SAVE_BUFFER[idx_TRUE_Slip + i][cnt] = true;
		} else {
		  SAVE_BUFFER[idx_TRUE_Slip + i][cnt] = false;
		}
		...
		for (int k = 0; k < 3; k++) {
		  ...
		  if (cnt >= WINDOW_SIZE) {
			...
			SAVE_BUFFER[idx_TRUE_dv + 3 * i + k][cnt] = dv_true(k);
		  }
		}
	  }

	  for (int i = 0; i < estimator_common_struct_.leg_no; i++) {
		SAVE_BUFFER[idx_TRUE_Contact + i][cnt] = GroundTruth[j + gt_sd][19 + i];

		SAVE_BUFFER[idx_TRUE_Hard_Contact + i][cnt] = RS[frame_count].Hard_Contact(i);
	  }
...
void InvariantSmoother::DoSaveAll(const std::string &cov_info) {

  std::cout << "DO SAVE_BUFFER ALL" << std::endl;

  FILE *ffp = nullptr;
  std::string str("../src/raisin_hound/houndlib/Invariant_Smoother_for_Legged_Robot.git/result/");



//    str = str + estimator_info +"_"+ cov_info + "_" + initial_info + "_" + file_info + "x" + std::to_string(time_count) + "_" + std::to_string(WINDOW_SIZE) + ".txt";

  str = str + "Estimation_Result_IS.txt";
...
void InvariantSmoother::Onestep(Eigen::Matrix<double, num_z, 1> Sensor_i,
    ...
    for (int kkk = 0; kkk < 4; kkk++) {
        ...
        foot_pos.setZero();
    foot_pos = LeggedRobotKinematics::GetFootPosition(kkk, Sensor_i.block(6 + 3 * (kkk - 1), 0, 3, 1));
//    std::cout << "foot "<< kkk << " th ... "<< foot_pos.transpose() << std::endl;
}
    new_measurement(Sensor_i, Contact_i, X_GPS_i, sqrt_GPS_i, GPS_In_i);
...
void InvariantSmoother::Onestep(Eigen::Matrix<double, num_z, 1> Sensor_i,
  ...
  new_measurement(Sensor_i, Contact_i, X_GPS_i, sqrt_GPS_i, GPS_In_i, X_LVM_i, sqrt_LVM_i, LVM_In_i);
