...
// ------------ InEKF -------------
// Default constructor
InEKF::InEKF() : g_((Eigen::VectorXd(3) << 0,0,-9.80665).finished()),
...
// Set LiDAR odometry data
void InEKF::setLidarOdometry(const Eigen::Matrix3d& rotation, const Eigen::Vector3d& position) {
    lidar_rotation_ = rotation;
    lidar_position_ = position;
}
...
void InEKF::Propagate(const Eigen::Matrix<double,6,1>& m, double dt, std::vector<Eigen::Vector3d> contact_cov_array) {
    ...
    A.block<3,3>(6,0) = 0.5 * Hat_so3(g_) * dt; //added by Z1
    // Bias terms
    ...
    Qk.block<3,3>(6,6) = noise_params_.getAccelerometerCov() * (0.5*dt*dt); // Added by Z1
    for(map<int,int>::iterator it=estimated_contact_positions_.begin(); it!=estimated_contact_positions_.end(); ++it) {
        Qk.block<3,3>(3+3*(it->second-3),3+3*(it->second-3)).diagonal() = contact_cov_array[it->first]; // Contact noise terms
    }
...
void InEKF::CorrectKinematics(const vectorKinematics& measured_kinematics, bool flag, bool lidar_available, Eigen::Vector3d lidar_position) {
#if INEKF_USE_MUTEX
    lock_guard<mutex> mlock(estimated_contacts_mutex_);
#endif
    Eigen::VectorXd Y;
...
void InEKF::CorrectKinematicsAndLidar
(
    const vectorKinematics& measured_kinematics, bool flag, bool lidar_available, 
    Eigen::Vector3d lidar_position, bool gps_available, Eigen::Vector3d gps_position
) {

#if INEKF_USE_MUTEX
    lock_guard<mutex> mlock(estimated_contacts_mutex_);
#endif
    Eigen::VectorXd Y_kin, Y_lid, Y_gps, Y;
    Eigen::VectorXd b_kin, b_lid, b_gps, b;
    Eigen::MatrixXd H_kin, H_lid, H_gps, H;
    Eigen::MatrixXd N_kin, N_lid, N_gps, N;
    Eigen::MatrixXd PI_kin,PI_lid,PI_gps,PI;

    Eigen::Matrix3d R = state_.getRotation();
    Eigen::Vector3d p = state_.getPosition();

    Eigen::Vector3d lidar_position_base = R.transpose() * (lidar_position);

    Eigen::Vector3d gps_position_base = R.transpose() * (gps_position);
    // std::cout << "gps_position: " << gps_position << std::endl;
    // std::cout << "gps_position_base: " << gps_position_base << std::endl;
   ...
   for (vectorKinematicsIterator it=measured_kinematics.begin(); it!=measured_kinematics.end(); ++it) {
        ...
        // If contact is indicated and id is found in estimated_contacts_, then correct using kinematics
            ...
            int startIndex;
            // Fill out Y
            startIndex = Y_kin.rows();
            Y_kin.conservativeResize(startIndex+dimX, Eigen::NoChange);
            Y_kin.segment(startIndex,dimX) = Eigen::VectorXd::Zero(dimX);
            Y_kin.segment(startIndex,3) = it->pose.block<3,1>(0,3); // p_bc
            Y_kin(startIndex+4) = 1; 
            Y_kin(startIndex+it_estimated->second) = -1; 

            Y_lid.conservativeResize(startIndex+dimX, Eigen::NoChange);
            Y_lid.segment(startIndex,dimX) = Eigen::VectorXd::Zero(dimX);
            Y_lid.segment(startIndex,3) = -lidar_position_base; // p_bl
            Y_lid(startIndex+4) = 1;
            Y_lid(startIndex+it_estimated->second) = 0; // p_bl is not affected by contact point

            // GPS
            Y_gps.conservativeResize(startIndex+dimX, Eigen::NoChange);
            Y_gps.segment(startIndex,dimX) = Eigen::VectorXd::Zero(dimX);
            Y_gps.segment(startIndex,3) = -gps_position_base; // p_gps
            Y_gps(startIndex+4) = 1;
            Y_gps(startIndex+it_estimated->second) = 0; // p_gps is not affected by contact point

            // Fill out b
            startIndex = b_kin.rows();
            b_kin.conservativeResize(startIndex+dimX, Eigen::NoChange);
            b_kin.segment(startIndex,dimX) = Eigen::VectorXd::Zero(dimX);
            b_kin(startIndex+4) = 1;     
            b_kin(startIndex+it_estimated->second) = -1;

            b_lid.conservativeResize(startIndex+dimX, Eigen::NoChange);
            b_lid.segment(startIndex,dimX) = Eigen::VectorXd::Zero(dimX);
            b_lid(startIndex+4) = 1;
            b_lid(startIndex+it_estimated->second) = 0; // p_bl is not affected by contact point

            b_gps.conservativeResize(startIndex+dimX, Eigen::NoChange);
            b_gps.segment(startIndex,dimX) = Eigen::VectorXd::Zero(dimX);
            b_gps(startIndex+4) = 1;
            b_gps(startIndex+it_estimated->second) = 0;

            // Fill out H
            startIndex = H_kin.rows();
            H_kin.conservativeResize(startIndex+3, dimP);
            H_kin.block(startIndex,0,3,dimP) = Eigen::MatrixXd::Zero(3,dimP);
            H_kin.block(startIndex,6,3,3) = -Eigen::Matrix3d::Identity(); // -I
            H_kin.block(startIndex,3*it_estimated->second-6,3,3) = Eigen::Matrix3d::Identity(); // I

            H_lid.conservativeResize(startIndex+3, dimP);
            H_lid.block(startIndex,0,3,dimP) = Eigen::MatrixXd::Zero(3,dimP);
            H_lid.block(startIndex,6,3,3) = -Eigen::Matrix3d::Identity(); // -I

            H_gps.conservativeResize(startIndex+3, dimP);
            H_gps.block(startIndex,0,3,dimP) = Eigen::MatrixXd::Zero(3,dimP);
            H_gps.block(startIndex,6,3,3) = -Eigen::Matrix3d::Identity(); // -I

            // Fill out N
            startIndex = N_kin.rows();
            N_kin.conservativeResize(startIndex+3, startIndex+3);
            N_kin.block(startIndex,0,3,startIndex) = Eigen::MatrixXd::Zero(3,startIndex);
            N_kin.block(0,startIndex,startIndex,3) = Eigen::MatrixXd::Zero(startIndex,3);
            N_kin.block(startIndex,startIndex,3,3) = R * it->covariance.block<3,3>(3,3) * R.transpose();

            N_lid.conservativeResize(startIndex+3, startIndex+3);
            N_lid.block(startIndex,0,3,startIndex) = Eigen::MatrixXd::Zero(3,startIndex);
            N_lid.block(0,startIndex,startIndex,3) = Eigen::MatrixXd::Zero(startIndex,3);
            Eigen::Matrix3d cov_lidar; 
            cov_lidar.setIdentity();
            cov_lidar << 1e-1, 0.0, 0.0,    // indoor:  1e-2, 0.0, 0.0,   // outdoor:  1e-1, 0.0, 0.0,          
                         0.0, 1e-1, 0.0,    //          0.0, 1e-2, 0.0,   //           0.0, 1e-1, 0.0,                        
                         0.0, 0.0, 1e-1;     //          0.0, 0.0, 1e-2;   //           0.0, 0.0, 1e3;                    
            N_lid.block(startIndex,startIndex,3,3) = R * (cov_lidar) * R.transpose();

            N_gps.conservativeResize(startIndex+3, startIndex+3);
            N_gps.block(startIndex,0,3,startIndex) = Eigen::MatrixXd::Zero(3,startIndex);
            N_gps.block(0,startIndex,startIndex,3) = Eigen::MatrixXd::Zero(startIndex,3);
            Eigen::Matrix3d cov_gps; 
            cov_gps.setIdentity();
            cov_gps << 1e-3, 0.0, 0.0,       //   indoor: 1e-2, 0.0, 0.0,  
                       0.0, 1e-3, 0.0,       //           0.0, 1e-2, 0.0,  
                       0.0, 0.0, 1.e-3;       //           0.0, 0.0, 1e-7;     
            N_gps.block(startIndex,startIndex,3,3) = R * cov_gps * R.transpose();

            // Fill out PI      
            startIndex = PI_kin.rows();
            int startIndex2 = PI_kin.cols();
            PI_kin.conservativeResize(startIndex+3, startIndex2+dimX);
            PI_kin.block(startIndex,0,3,startIndex2) = Eigen::MatrixXd::Zero(3,startIndex2);
            PI_kin.block(0,startIndex2,startIndex,dimX) = Eigen::MatrixXd::Zero(startIndex,dimX);
            PI_kin.block(startIndex,startIndex2,3,dimX) = Eigen::MatrixXd::Zero(3,dimX);
            PI_kin.block(startIndex,startIndex2,3,3) = Eigen::Matrix3d::Identity();

            PI_lid.conservativeResize(startIndex+3, startIndex2+dimX);
            PI_lid.block(startIndex,0,3,startIndex2) = Eigen::MatrixXd::Zero(3,startIndex2);
            PI_lid.block(0,startIndex2,startIndex,dimX) = Eigen::MatrixXd::Zero(startIndex,dimX);
            PI_lid.block(startIndex,startIndex2,3,dimX) = Eigen::MatrixXd::Zero(3,dimX);
            PI_lid.block(startIndex,startIndex2,3,3) = Eigen::Matrix3d::Identity();

            PI_gps.conservativeResize(startIndex+3, startIndex2+dimX);
            PI_gps.block(startIndex,0,3,startIndex2) = Eigen::MatrixXd::Zero(3,startIndex2);
            PI_gps.block(0,startIndex2,startIndex,dimX) = Eigen::MatrixXd::Zero(startIndex,dimX);
            PI_gps.block(startIndex,startIndex2,3,dimX) = Eigen::MatrixXd::Zero(3,dimX);
            PI_gps.block(startIndex,startIndex2,3,3) = Eigen::Matrix3d::Identity();


            if (lidar_available && gps_available) {

                // std::cout << "lidar and gps available" << std::endl;
                Y.conservativeResize(Y_kin.rows() + Y_lid.rows() + Y_gps.rows(), Eigen::NoChange);
                Y << Y_kin, Y_lid, Y_gps;

                b.conservativeResize(b_kin.rows() + b_lid.rows() + b_gps.rows(), Eigen::NoChange);
                b << b_kin, b_lid, b_gps;

                H.conservativeResize(H_kin.rows() + H_lid.rows() + H_gps.rows(), H_kin.cols());
                H.setZero();
                H.block(0, 0, H_kin.rows(), H_kin.cols()) = H_kin;
                H.block(H_kin.rows(), 0, H_lid.rows(), H_lid.cols()) = H_lid;
                H.block(H_kin.rows() + H_lid.rows(), 0, H_gps.rows(), H_gps.cols()) = H_gps;

                N.conservativeResize(N_kin.rows() + N_lid.rows() + N_gps.rows(), N_kin.cols() + N_lid.cols() + N_gps.cols());
                N.setZero();
                N.block(0, 0, N_kin.rows(), N_kin.cols()) = N_kin;
                N.block(N_kin.rows(), N_kin.cols(), N_lid.rows(), N_lid.cols()) = N_lid;
                N.block(N_kin.rows() + N_lid.rows(), N_kin.cols() + N_lid.cols(), N_gps.rows(), N_gps.cols()) = N_gps;

                PI.conservativeResize(PI_kin.rows() + PI_lid.rows() + PI_gps.rows(), PI_kin.cols() + PI_lid.cols() + PI_gps.cols());
                PI.setZero();
                PI.block(0, 0, PI_kin.rows(), PI_kin.cols()) = PI_kin;
                PI.block(PI_kin.rows(), PI_kin.cols(), PI_lid.rows(), PI_lid.cols()) = PI_lid;
                PI.block(PI_kin.rows() + PI_lid.rows(), PI_kin.cols() + PI_lid.cols(), PI_gps.rows(), PI_gps.cols()) = PI_gps;

            } else if (lidar_available) {

                std::cout << "lidar available" << std::endl;
                // startIndex = Y.rows();
                Y.conservativeResize(Y_kin.rows() + Y_lid.rows(), Eigen::NoChange);
                Y << Y_kin, Y_lid;

                b.conservativeResize(b_kin.rows() + b_lid.rows(), Eigen::NoChange);
                b << b_kin, b_lid;

                H.conservativeResize(H_kin.rows() + H_lid.rows(), H_kin.cols());
                H.setZero();
                H.block(0, 0, H_kin.rows(), H_kin.cols()) = H_kin;
                H.block(H_kin.rows(), 0, H_lid.rows(), H_lid.cols()) = H_lid;

                N.conservativeResize(N_kin.rows() + N_lid.rows(), N_kin.cols() + N_lid.cols());
                N.setZero();
                N.block(0, 0, N_kin.rows(), N_kin.cols()) = N_kin;
                N.block(N_kin.rows(), N_kin.cols(), N_lid.rows(), N_lid.cols()) = N_lid;

                PI.conservativeResize(PI_kin.rows() + PI_lid.rows(), PI_kin.cols() + PI_lid.cols());
                PI.setZero();
                PI.block(0, 0, PI_kin.rows(), PI_kin.cols()) = PI_kin;
                PI.block(PI_kin.rows(), PI_kin.cols(), PI_lid.rows(), PI_lid.cols()) = PI_lid;
                ...
                // std::cout << "gps available" << std::endl;
                // startIndex = Y.rows();
                Y.conservativeResize(Y_kin.rows() + Y_gps.rows(), Eigen::NoChange);
                Y << Y_kin, Y_gps;

                b.conservativeResize(b_kin.rows() + b_gps.rows(), Eigen::NoChange);
                b << b_kin, b_gps;

                H.conservativeResize(H_kin.rows() + H_gps.rows(), H_kin.cols());
                H.setZero();
                H.block(0, 0, H_kin.rows(), H_kin.cols()) = H_kin;
                H.block(H_kin.rows(), 0, H_gps.rows(), H_gps.cols()) = H_gps;

                N.conservativeResize(N_kin.rows() + N_gps.rows(), N_kin.cols() + N_gps.cols());
                N.setZero();
                N.block(0, 0, N_kin.rows(), N_kin.cols()) = N_kin;
                N.block(N_kin.rows(), N_kin.cols(), N_gps.rows(), N_gps.cols()) = N_gps;


                PI.conservativeResize(PI_kin.rows() + PI_gps.rows(), PI_kin.cols() + PI_gps.cols());
                PI.setZero();
                PI.block(0, 0, PI_kin.rows(), PI_kin.cols()) = PI_kin;
                PI.block(PI_kin.rows(), PI_kin.cols(), PI_gps.rows(), PI_gps.cols()) = PI_gps;
            }
            else {

                // std::cout << "no gps and lidar available" << std::endl;
                Y = Y_kin;
                b = b_kin;
                H = H_kin;
                N = N_kin;
    ...
    // Correct state using stacked observation
    Observation obs(Y,b,H,N,PI);
    Observation obs_kin(Y_kin,b_kin,H_kin,N_kin,PI_kin);
    Observation obs_lid(Y_lid,b_lid,H_lid,N_lid,PI_lid);
    ...
    // if(flag && lidar_available){
    //     if (!obs_lid.empty()) {
    //         // std::cout << "LIDAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAARRRRRRRRRRRR" << std::endl;
    //         this->Correct(obs_lid);
    //     }
    // }
    // else
    // if(flag){
    //     if (!obs_kin.empty()) {
    //         this->Correct(obs_kin);
    //     }
    // }
